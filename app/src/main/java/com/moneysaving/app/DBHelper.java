...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        try {
            ...
            db.execSQL("CREATE TABLE `images` (  `_id`	INTEGER PRIMARY KEY AUTOINCREMENT,userid INTEGER,imguri  TEXT)");
        } catch (Exception ex) {
    ...
    public long insertImgDb(int uid, String img) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        long rowinserted = 0;
        try {
            cv.put("userid", uid);
            cv.put("imguri", img);
            rowinserted = db.insert("images", null, cv);
    ...
    public Cursor get_img(int iid) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c1 = null;
        try {
            c1 = db.rawQuery("select imguri from images where userid = " + iid + "   order by _id desc", null);
